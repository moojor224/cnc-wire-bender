...
export class Breadboard {
    ...
    constructor(config = {}) {
        ...
        let individual_width =
            (power_rails.enabled ? 2 * s * (power_rails.num_cols + power_rails.main_spacing) : 0) +
        ...
        this.total_length =
            individual_length * this.config.num_breadboards_y +
            this.config.breadboard_y_gap * s * this.config.num_breadboards_y;
        this.total_width =
            individual_width * this.config.num_breadboards_x + this.config.breadboard_x_gap * s * this.config.num_breadboards_x;
        this.insert_shell();
        let xOffset = 0;
        let yOffset = 0;
                ...
                if (this.config.breadboard_y_gap > 0 && y > 0) {
                    this.#g.push(
                        q(
                            ...
                            v(
                                xOffset + xgap * s * (x == xmax - 1 ? 0 : 1) + individual_width,
                                (ygap * s) / 2 + ygap * s * (y - 1) + y * individual_length,
                                0
                            ),
                            v(
                                xOffset + xgap * s * (x == xmax - 1 ? 0 : 1) + individual_width,
                                (ygap * s) / 2 + ygap * s * (y - 1) + y * individual_length + this.config.breadboard_y_gap * s,
                                0
                            ),
            ...
            if (this.config.breadboard_x_gap > 0 && x > 0) {
                ...
                this.#g.push(
                    q(
                        ...
                        v(
                            (xgap * s) / 2 + xgap * s * (x - 1) + x * individual_width + xgap * s,
                            (ygap * s) / 2 + this.total_length - ygap * s,
                            0
                        ),
                        v(
                            (xgap * s) / 2 + xgap * s * (x - 1) + x * individual_width,
                            (ygap * s) / 2 + this.total_length - ygap * s,
                            0
                        ),
        ...
        this.#g.push(
            q(v(0, 0, 0), v(this.total_width, 0, 0), v(this.total_width, (ygap * s) / 2, 0), v(0, (ygap * s) / 2, 0)),
            q(
                v(0, (ygap * s) / 2, 0),
    ...
    hole(xCoord = 0, yCoord = 0, xOffset = 0, yOffset = 0) {
        ...
        // hole floor
        g.push(
            q(v(x + ip, y + ip, -id), v(x + s - ip, y + ip, -id), v(x + s - ip, y + s - ip, -id), v(x + ip, y + s - ip, -id))
        ); // floor
        this.#holes.push({ x, y, width: s, height: s });

        this.#face_plate_holes.push(
            q(v(x + op, y + op, 0.1), v(x + op, y + s - op, 0.1), v(x + s - op, y + s - op, 0.1), v(x + s - op, y + op, 0.1))
        );
        return g;
    }
    #face_plate = [];
    ...
    get face_plate() {
        ...
        // })));
        let holes = new M(
            BufferGeometryUtils.mergeGeometries(this.#face_plate_holes),
    ...
    subsection(xOff, yOff, h_index) {
        let {
            config: { hole_spacing: s, holes_per_row, rows_per_section },
            module_length
        } = this;
